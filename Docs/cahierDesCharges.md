# Cahier des Charges - Projet Gamer Challenges

## Table des matières
- [Cahier des Charges - Projet Gamer Challenges](#cahier-des-charges---projet-gamer-challenges)
  - [Table des matières](#table-des-matières)
  - [1. Présentation du projet](#1-présentation-du-projet)
  - [2. Analyse des besoins](#2-analyse-des-besoins)
    - [2.1 Besoins fonctionnels](#21-besoins-fonctionnels)
    - [2.2 Cas d'usage](#22-cas-dusage)
  - [3. Spécifications fonctionnelles](#3-spécifications-fonctionnelles)
    - [3.1 Architecture de l'information](#31-architecture-de-linformation)
    - [3.2 API et endpoints](#32-api-et-endpoints)
    - [3.3 Workflows principaux](#33-workflows-principaux)
  - [4. Design et expérience utilisateur](#4-design-et-expérience-utilisateur)
    - [4.1 Charte graphique](#41-charte-graphique)
    - [4.2 Wireframes](#42-wireframes)
    - [4.3 Maquettes](#43-maquettes)
  - [5. Spécifications techniques](#5-spécifications-techniques)
    - [5.1 Contraintes techniques](#51-contraintes-techniques)
    - [5.2 Modèle de données](#52-modèle-de-données)
  - [6. Gestion des risques](#6-gestion-des-risques)
  - [7. Plan de tests](#7-plan-de-tests)
  - [8. Annexes](#8-annexes)
    - [8.1 Documents de référence](#81-documents-de-référence)
    - [8.2 Diagrammes et modèles](#82-diagrammes-et-modèles)
    - [8.3 Ressources design](#83-ressources-design)
    - [8.4 Glossaire](#84-glossaire)

---

## 1. Présentation du projet

📋 **Document détaillé :** [Présentation du projet](./CahierDesCharges/PresentationProjet.md)

📋 **Document détaillé :** [Liste des rôles](./CahierDesCharges/ListesDesRoles.md)

---

## 2. Analyse des besoins

### 2.1 Besoins fonctionnels
📋 **Document détaillé :** [User Stories](./CahierDesCharges/UserStories.md)

### 2.2 Cas d'usage
📋 **Documents détaillés :**
- [Use Cases](./Conception/UseCases/UseCases.md)
- [Diagramme Use Cases](./Conception/UseCases/UseCases.puml)

---

## 3. Spécifications fonctionnelles

### 3.1 Architecture de l'information
📋 **Document détaillé :** [Arborescence](./CahierDesCharges/Arborescence.md)

### 3.2 API et endpoints
📋 **Document détaillé :** [Endpoints](./CahierDesCharges/Endpoints.md)

### 3.3 Workflows principaux
📋 **Document détaillé :** [Diagramme de séquence](./Conception/Diagrammes/seqDiag1.puml)

---

## 4. Design et expérience utilisateur

### 4.1 Charte graphique
📋 **Document détaillé :** [Charte graphique](./Conception/CharteGraphique/CharteGraphique.md)

La charte graphique définit l'identité visuelle de la plateforme Gamer Challenges.

### 4.2 Wireframes
📋 **Documents détaillés :**
- [Wireframes desktop](./Conception/Wireframes/Wireframes%20Desktop.pdf)
- [Wireframes mobile](./Conception/Wireframes/Wireframes%20Mobile.pdf)

### 4.3 Maquettes
📋 **Documents détaillés :**
- [Maquettes desktop](./Design/Maquettes/Desktop/) *(À créer)*
- [Maquettes mobile](./Design/Maquettes/Mobile/) *(À créer)*
- [Prototypes interactifs](./Design/Maquettes/Prototypes/) *(À créer)*

---

## 5. Spécifications techniques

### 5.1 Contraintes techniques
📋 **Document détaillé :** [Contraintes techniques](./CahierDesCharges/ContraintesTechniques.md)

### 5.2 Modèle de données
📋 **Documents détaillés :**
- [Dictionnaire de données](./Conception/dictionnaireDeDonnées.md)
- [MCD](./Conception/MCD/MCD.md)
- [MLD](./Conception/MLD/MLDv2.drawio)
- [ERD](./Conception/ERD/ERD.puml)

---

## 6. Gestion des risques

📋 **Document détaillé :** [Analyse des risques](./CahierDesCharges/AnalyseDesRiques.md)

---

## 7. Plan de tests

📋 **Document détaillé :** [Plan de tests](./Conception/PlanTests/PlanTests.md)

---

## 8. Annexes

### 8.1 Documents de référence
- [User Stories détaillées](./CahierDesCharges/UserStories.md)
- [Spécifications techniques complètes](./CahierDesCharges/ContraintesTechniques.md)
- [Documentation de l'API](./CahierDesCharges/Endpoints.md)
- [Plan de tests complet](./Conception/PlanTests/PlanTests.md)

### 8.2 Diagrammes et modèles
- [Diagrammes UML](./Conception/)
- [Modèles de données](./Conception/MCD/)
- [Architecture système](./Conception/ERD/)

### 8.3 Ressources design
- [Charte graphique complète](./Conception/CharteGraphique/CharteGraphique.md)
- [Bibliothèque de wireframes](./Conception/Wireframes/) 
- [Kit de maquettes](./Design/Maquettes/) *(À créer)*

### 8.4 Glossaire
- **Challenge** : Compétition organisée sur un jeu spécifique
- **Ranking** : Classement basé sur les performances
- **Organisateur** : Utilisateur autorisé à créer des challenges
- **Performance** : Résultat d'un participant à un challenge
- **Wireframe** : Maquette fonctionnelle basse-fidélité
- **Mockup** : Maquette graphique haute-fidélité
- **UI/UX** : Interface utilisateur et expérience utilisateur
- **Test unitaire** : Test d'un composant isolé
- **Test d'intégration** : Test des interactions entre composants

---